...
- (**Coming soon** )`MEDIA_RESOLUTION_ULTRA_HIGH`: Highest token count required for specific use cases such as computer use.
...
- **Per-part control (Gemini 3):** Leverage this to optimize token usage. For example, in a prompt with multiple images, use`HIGH`for a complex diagram and`LOW`or`MEDIUM`for simpler contextual images.
